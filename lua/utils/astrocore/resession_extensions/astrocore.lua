...
function M.on_save(opts)
  ...
  local buf_utils = require("utils.astrocore.buffer")
...
function M.on_post_load(data)
  ...
  if current_buf and last_buf then
    if vim.opt.bufhidden:get() == "wipe" and vim.fn.bufnr() ~= buf_utils.current_buf then vim.cmd.b(buf_utils.current_buf) end
  end
end
