...
local function notify(msg, level) require("utils.astrocore").notify(msg, level or vim.log.levels.INFO, { title = "AstroNvim Rooter" }) end

local function resolve_config() return require("utils.astrocore").config.rooter or {} end

--- Create a detect workspace folders from active language servers
---@param config AstroCoreRooterOpts? a 7ooter configuration (defaults to global configuration)
...
function M.detect(bufnr, all, config)
  ...
  if not require("utils.astrocore.buffer").is_valid(bufnr) then return ret end
  ...
  if vim_autochdir then
    ...
    for _, root in ipairs(roots) do
      for _, path in ipairs(root.paths) do
        ...
        table.insert(lines, ("%s`%s` *(%s*)%s"):format(surround, path, type(root.spec) == "table" and table.concat(root.spec --[=[@as string[]]=], ", ") or root.spec, surround))
        first = false
      end
    end
