...
function M.is_restorable(bufnr)
  ...
  if vim.tbl_contains(M.sessions.ignore.filetypes, vim.bo[bufnr].filetype) or vim.tbl_contains(M.sessions.ignore.buftypes, vim.bo[bufnr].buftype) then return false end
  return true
end
...
function M.sort(compare_func, skip_autocmd)
  if type(compare_func) == "string" then compare_func = require("utils.astrocore.buffer.comparator")[compare_func] end
