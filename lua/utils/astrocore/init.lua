...
---
---This module can be loaded with `local astro = require "utils.astrocore"`
---
---copyright 2023
...
M.config = require("utils.astrocore.config")
...
function M.update_packages()
  ...
  require("utils.astrocore.mason").update_all()
end
...
function M.setup(opts)
  ...
  M.diagnostics = {
    -- diagnostics off
    [0] = vim.tbl_deep_extend("force", M.config.diagnostics, { underline = false, virtual_text = false, signs = false, update_in_insert = false }) --[[@as vim.diagnostic.Opts]],
    -- status only
  ...
  if large_buf then
    vim.api.nvim_create_autocmd("BufRead", {
      ...
      callback = function(args)
        ...
        if (ok and stats and stats.size > large_buf.size) or vim.api.nvim_buf_line_count(args.buf) > large_buf.lines then
  ...
  vim.api.nvim_create_autocmd({ "VimEnter", "FileType", "BufEnter", "WinEnter" }, {
    ...
    callback = function(args)
      for _, win in ipairs(vim.api.nvim_list_wins()) do
        if vim.api.nvim_win_get_buf(win) == args.buf and vim.tbl_get(require("astrocore"), "config", "features", "highlighturl") and not vim.w[win].highlighturl_enabled then
          M.set_url_match(win)
        end
      end
    end,
  ...
  if vim.tbl_get(M.config, "rooter", "enabled") then
    ...
    vim.api.nvim_create_user_command("AstroRootInfo", function() require("utils.astrocore.rooter").info() end, { desc = "Display rooter information" })
    vim.api.nvim_create_user_command("AstroRoot", function() require("utils.astrocore.rooter").root() end, { desc = "Run root detection" })
    ...
    vim.api.nvim_create_autocmd({ "VimEnter", "BufEnter" }, {
      ...
      desc = "Root detection when entering a buffer",
      callback = function(args)
        if root_config.autochdir then require("utils.astrocore.rooter").root(args.buf) end
      end,
    })
      ...
      vim.api.nvim_create_autocmd("LspAttach", {
        ...
        callback = function(args)
          if root_config.autochdir then
            ...
            if not vim.tbl_contains(vim.tbl_get(root_config, "ignore", "servers") or {}, server) then require("utils.astrocore.rooter").root(args.buf) end
          end
        end,
      })
    end
