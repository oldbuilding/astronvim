...
---
---This module can be loaded with `local ui = require("utils.astrocore.toggles")`
---
---copyright 2023
---license GNU General Public License v3.0
---@class utils.astrocore.toggles
local M = {}

local function bool2str(bool) return bool and "on" or "off" end
local function ui_notify(silent, ...) return not silent and require("utils.astrocore").notify(...) end

--- Toggle rooter autochdir
---@param silent? boolean if true then don't sent a notification
function M.autochdir(silent)
  local root_config = require("utils.astrocore").config.rooter
...
function M.notifications(silent)
  local features = assert(require("utils.astrocore").config.features)
...
function M.autopairs(silent)
  ...
  if ok then
    ...
    require("utils.astrocore").config.features.autopairs = autopairs.state.disabled
...
function M.buffer_cmp(bufnr, silent)
  ...
  if vim.b[bufnr].cmp_enabled == nil then vim.b[bufnr].cmp_enabled = require("utils.astrocore").config.features.cmp end
  vim.b[bufnr].cmp_enabled = not vim.b[bufnr].cmp_enabled
  local ok, _ = pcall(require, "cmp")
  ui_notify(silent, ok and ("Buffer completion %s"):format(bool2str(vim.b[bufnr].cmp_enabled)) or "completion not available")
end
...
function M.url_match(silent)
  ...
  vim.tbl_map(require("utils.astrocore").set_url_match, vim.api.nvim_list_wins())
...
function M.diagnostics(silent)
  local features = require("utils.astrocore").config.features --[[@as AstroCoreFeatureOpts]]
  features.diagnostics_mode = (features.diagnostics_mode - 1) % 4
  vim.diagnostic.config(require("utils.astrocore").diagnostics[features.diagnostics_mode])
